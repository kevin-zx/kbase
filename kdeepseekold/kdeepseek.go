// Package kdeepseekold 是 DeepSeek API 的旧版 Go 客户端封装。
// 新项目请使用 github.com/kevin-zx/kbase/kdeepseek。
package kdeepseekold
...
// ChatOption 用于在 NewChat 时配置 ChatConfig。
...
func (c *Client) NewChat(systemPrompt string, opts ...ChatOption) *Chat {
	config := ChatConfig{}
...
// Send 发送一条用户消息并返回助手回复文本，自动维护历史和锁。
func (ch *Chat) Send(prompt string) (string, error) {
	...
	resp, err := ch.client.CreateChatCompletion(req)
	if err != nil {
...
func (ch *Chat) SendStream(ctx context.Context, prompt string) (<-chan StreamChunk, <-chan error) {
	...
	go func() {
		...
		httpReq, err := http.NewRequestWithContext(
			ctx,
			"POST",
			fmt.Sprintf("%s/chat/completions", ch.client.baseURL),
			bytes.NewReader(payload),
		)
		if err != nil {
			ch.history = ch.history[:len(ch.history)-1]
		...
		httpReq.Header.Set("Content-Type", "application/json")
		httpReq.Header.Set("Accept", "text/event-stream")
		httpReq.Header.Set("Authorization", "Bearer "+ch.client.token)

		resp, err := ch.client.httpClient.Do(httpReq)
		if err != nil {
			ch.history = ch.history[:len(ch.history)-1]
			errCh <- fmt.Errorf("error sending request: %w", err)
			return
